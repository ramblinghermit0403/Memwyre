...
def find_line_number(content, tag_str):
    # Try to find the line number of a tag
    # Just take first 50 chars of tag_str to handle formatting changes
    search_str = tag_str[:50].strip()
...
def parse_vue(file_path, route):
    ...
    for rl in soup.find_all(["router-link", "routerlink"]):
        ...
        if to_val:
            ...
            # Reconstruct rough tag prefix to find it in file
            tag_str = str(rl)
            line_num = find_line_number(content, tag_str)
