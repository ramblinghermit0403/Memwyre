...
ROUTES = {
    ...
    "/redeem", "/slides", "/login", "/signup", "/forgot-password", "/verify-email",
    "/reset-password"
}

# Directories that need trailing slashes
INDEX_ROUTES = {"/docs/", "/docs/integrations/"}
...
def audit_link(file_path, line_num, source_route, target_url, link_text):
    if not target_url or target_url == "None" or target_url.startswith("`") or target_url.startswith("post.") or target_url.startswith("item.") or "{." in target_url or "{{" in target_url:
        return
        
    if target_url in ["product.to", "link.to", "item.link", "post.slug"]:
        return
    ...
    if not target_url.startswith("/"):
        # Correctly determine base directory route
        if source_route.endswith("/"):
            base = source_route
        elif "/" in source_route:
    ...
    if path_only in ROUTES:
        is_valid = True
        # Check redirect
        if path_only in INDEX_ROUTES:
            if not path_only.endswith("/"):
                redirect_reason = f"Redirects to {path_only}/ (Index requires trailing slash)"
        else:
            if path_only != "/" and path_only.endswith("/"):
                redirect_reason = f"Redirects to {path_only[:-1]} (Clean URL/SPA strips trailing slash)"
...
# Write to JSON
import json
with open(r"c:\Users\himan\OneDrive\Documents\brain_vault\scratch\detailed_broken_links_final_v6.json", "w", encoding="utf-8") as f:
    json.dump(unique_issues, f, indent=2)

print(f"Total Unique Issues Found: {len(unique_issues)}")
for issue in unique_issues:
    desc = issue.get("description", "")
    # Safe console printing by replacing unicode arrows
    anchor = issue['anchor'].replace("\u2192", "->")
    print(f"[{issue['severity']}] {issue['type']} in {issue['file']}:{issue['line']} -> target: {issue['target']} (text: '{anchor}') {desc}")
