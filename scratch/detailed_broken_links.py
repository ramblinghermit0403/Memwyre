...
def audit_link(file_path, line_num, source_route, target_url, link_text):
    if not target_url or target_url == "None" or target_url.startswith("`") or target_url.startswith("post.") or target_url.startswith("item."):
        return
...
def parse_vue(file_path, route):
    ...
    for idx, line in enumerate(lines):
        ...
        # Match router-link
        rl_matches = re.finditer(r'<router-link\s+[^>]*to=["\']([^"\']*)["\'][^>]*>(.*?)</router-link>', line, re.IGNORECASE)
        for match in rl_matches:
            url = match.group(1).strip()
            text = re.sub(r'<[^>]+>', '', match.group(2)).strip()
            audit_link(file_path, line_num, route, url, text)
            
        # Match normal HTML a href links
        a_matches = re.finditer(r'<a\s+[^>]*href=["\']([^"\']*)["\'][^>]*>(.*?)</a>', line, re.IGNORECASE)
...
for issue in unique_issues:
    ...
    severity_text = issue['severity'].replace("🔴", "High").replace("🟠", "Medium").replace("🟡", "Low")
    print(f"[{severity_text}] {issue['type']} in {issue['file']}:{issue['line']} -> target: {issue['target']} (text: '{issue['anchor']}') {desc}")
