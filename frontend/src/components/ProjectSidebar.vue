  ...
  <div class="h-full flex flex-col">
    <div class="p-4 border-b border-gray-100 dark:border-gray-700 space-y-3">
      ...
      <div class="flex gap-2">
        <input v-model="newProjectName" @keyup.enter="create" class="flex-1 text-xs border border-gray-200 dark:border-gray-700 rounded-md px-2 py-1 bg-white dark:bg-surface" placeholder="New project" />
        <button @click="create" class="text-xs px-2 py-1 rounded-md bg-[#D97757] text-white">Add</button>
      </div>
    </div>
    <div class="flex-1 overflow-y-auto p-2 space-y-1">
      <button @click="$emit('select', null)" :class="buttonClass(selectedProjectId === null)">Unassigned</button>
